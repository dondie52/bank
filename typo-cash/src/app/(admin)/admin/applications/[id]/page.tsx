...
const application = {
  ...
  affordability: {
    grossIncome: 1200000n,
    deductions: 300000n,
    netIncome: 900000n,
    existingObligations: 150000n,
    disposableIncome: 750000n,
    proposedInstalment: 336000n,
    dtiRatio: 0.37,
    instalmentRatio: 0.25,
    passes: true,
  },
  riskScore: {
    overall: 72,
    components: [
      { label: "Credit History", score: 65 },
      { label: "Income Stability", score: 85 },
      { label: "Debt Ratio", score: 70 },
      { label: "Employment", score: 90 },
      { label: "Behavioural", score: 50 },
    ],
  },
};
...
export default function ApplicationDetailPage() {
  ...
  const cc = app.creditCheck;
  const af = app.affordability;
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            <h1 className="text-2xl font-bold text-slate-900">Application {app.id}</h1>
            <p className="text-sm text-slate-500 mt-0.5">
              {app.product} &middot; {app.date}
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-card p-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {([
              ["Omang Front", app.kyc.omangFront],
              ["Omang Back", app.kyc.omangBack],
              ["Selfie", app.kyc.selfie],
              ["Proof of Income", app.kyc.proofOfIncome],
              ["Bank Statement", app.kyc.bankStatement],
            ] as [string, boolean][]).map(([label, uploaded]) => (
              <div
                key={label}
                className={cn(
                  "flex flex-col items-center justify-center p-4 rounded-lg border-2 border-dashed text-center gap-2",
                  uploaded ? "border-emerald-200 bg-emerald-50" : "border-red-200 bg-red-50"
                )}
              >
                {/* eslint-disable-next-line jsx-a11y/alt-text */}
                <Image className={cn("w-8 h-8", uploaded ? "text-emerald-500" : "text-red-400")} />
                <span className="text-xs font-medium text-slate-700">{label}</span>
                <span
                  className={cn(
                    "text-[10px] font-semibold uppercase",
                    uploaded ? "text-emerald-600" : "text-red-600"
                  )}
                >
        ...
        <div className="flex items-center gap-2 mb-4">
          ...
          <span
            className={cn(
              ...
              app.riskScore.overall >= 80
                ? "bg-emerald-100 text-emerald-700"
                : app.riskScore.overall >= 60
                ? "bg-amber-100 text-amber-700"
                : "bg-red-100 text-red-700"
            )}
          >
            {app.riskScore.overall}
          </span>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
          <RadarChart components={app.riskScore.components} />
          <div className="space-y-3">
            {app.riskScore.components.map((comp) => (
              <div key={comp.label}>
                <div className="flex justify-between text-sm mb-1">
          ...
          <div className="space-y-3">
            ...
            <button
              className={cn(
                ...
                decision === "decline" && !reason.trim() && "opacity-50 cursor-not-allowed"
              )}
              disabled={decision === "decline" && !reason.trim()}
            >
